...
def audit(directory: Path, output: Path | None = None):
    ...
    result = {"status": "retrospective_diagnostic", "inputs": {
        name: sha(directory / name) for name in ("dataset.json.gz", "results.json.gz", "input.json.gz")},
    ...
    for stage, feature, direction in (("0", "log_offer_price", -1), ("20", "daily_volatility", 1)):
        ...
        model_aucs = {name: float(roc_auc_score(y, [row["probabilities"][name] for row in complete]))
                      for name in rows[0]["probabilities"] if name != "base_rate"}
        returns, minimums, events, pops = [], [], [], []
        for row in rows:
            listing = listings[row["id"]]
            bars = [bar for bar in listing["history"]["bars"] if bar["date"] >= listing["listing_date"]]
            if stage == "0":
        ...
        returns, minimums, events = np.array(returns), np.array(minimums), np.array(events, dtype=bool)
        yearly = []
        for coverage in data["quality"]["years"]:
            eligible = [row for row in data["stages"][stage] if row["listing_date"].startswith(str(coverage["year"]))]
            yearly.append({"year": coverage["year"], "coverage": coverage["price_coverage"],
                           "eligible": len(eligible), "event_rate": float(np.mean([row["event"] for row in eligible]))})
        stage_result = {"held_out": len(rows), "single_feature": feature, "direction": direction,
                        "ranking_cases": len(complete), "ranking_missing": len(rows) - len(complete),
                        "single_feature_auc": float(roc_auc_score(y, score)), "model_aucs_same_cases": model_aucs,
                        "return_proxy_cases": len(returns), "event_cases_with_return_proxy": int(events.sum()),
                        ...
                        "yearly_coverage_event_correlation": float(np.corrcoef([r['coverage'] for r in yearly], [r['event_rate'] for r in yearly])[0, 1]),
                        "yearly": yearly}
        if stage == "0":
            stage_result.update(median_first_close_offer_proxy=float(np.median(pops)),
