...
from .sources import save_json, sha
...
def cat_matrix(rows, features):
    return [[row["features"][key] if row["features"][key] is not None else float("nan")
...
class Transform:
    ...
    def fit(cls, rows, features):
        ...
        medians = [float(np.median(column[np.isfinite(column)])) if np.isfinite(column).any() else 0.0
                   for column in x.T]
        x = np.where(np.isfinite(x), x, medians)
        scale = x.std(axis=0)
        scale[scale < 1e-12] = 1
        categories = {key: sorted({row["features"][key] for row in rows}) for key in categorical}
        return cls(numeric, categorical, medians, x.mean(axis=0).tolist(), scale.tolist(), categories)
...
def inner_split(rows, fraction=.2):
    ...
    if len(fit) < 50 or len(validation) < 20 or len(set(labels(fit))) != 2 or len(set(labels(validation))) != 2:
        raise ValueError("Inner split needs mature training and validation examples of both classes")
    if {r["issuer_id"] for r in fit} & {r["issuer_id"] for r in validation}:
        raise ValueError("Issuer leakage in the inner split")
    return fit, validation, {"start": cutoff, "fit_count": len(fit), "validation_count": len(validation),
                              "fit_label_end": max(r["label_end"] for r in fit),
                              "purged": len(rows) - len(fit) - len(validation),
                              "fit_ids": [r["id"] for r in fit], "validation_ids": [r["id"] for r in validation]}
...
def fit_baseline(train, test, stage, kind, full=False):
    ...
    if stage and not full:
        ...
        transform = None
        model = (make_pipeline(StandardScaler(), LogisticRegression(**MODEL_CONFIG["logistic"]))
                 if kind == "logistic" else GradientBoostingClassifier(**MODEL_CONFIG["boosted_trees"]))
    else:
        transform = Transform.fit(train, features)
        x, xt = transform.apply(train), transform.apply(test)
        model = (LogisticRegression(**MODEL_CONFIG["logistic"])
                 if kind == "logistic" else GradientBoostingClassifier(**MODEL_CONFIG["boosted_trees"]))
    model.fit(x, labels(train))
    return model.predict_proba(xt)[:, 1], {"model": model, "transform": transform, "features": features}


def cat_model(config, protocol, seed, iterations):
    return CatBoostClassifier(**config, iterations=iterations, learning_rate=protocol["catboost_learning_rate"],
                              loss_function="Logloss", eval_metric="BrierScore", boosting_type="Ordered",
                              random_seed=seed, thread_count=2, allow_writing_files=False, verbose=False)
...
def train_catboost(train, test, features, protocol, target: Path):
    ...
    for config in protocol["catboost_candidates"]:
        ...
        model.fit(cat_matrix(fit, features), labels(fit), cat_features=categorical,
        ...
        trials.append({"config": config, "iterations": int(model.tree_count_),
    ...
    return np.mean(predictions, axis=0), {"inner_split": split, "trials": trials, "selected": winner,
                                         "seed_predictions": [p.tolist() for p in predictions], "files": files}


def tab_model(transform, config, protocol, seed):
    cat_start = 2 * len(transform.numeric)
    return TabNetClassifier(**config, cat_idxs=list(range(cat_start, cat_start + len(transform.categorical))),
                            cat_dims=[len(transform.categories[key]) + 1 for key in transform.categorical],
                            cat_emb_dim=2, seed=seed, verbose=0, device_name="cpu",
...
def tab_fit(model, transform, train, protocol, epochs, validation=None):
    ...
    while len(train) % batch_size == 1:
        batch_size -= 1
    kwargs = {"max_epochs": epochs, "batch_size": batch_size,
              "virtual_batch_size": protocol["tabnet_virtual_batch_size"], "num_workers": 0,
              "drop_last": False, "weights": 0, "compute_importance": False,
              "patience": protocol["tabnet_patience"] if validation else 0}
    if validation:
        kwargs.update(eval_set=[(transform.apply(validation, embedding=True), labels(validation))],
                      eval_name=["validation"], eval_metric=[BrierMetric])
...
def train_tabnet(train, test, features, protocol, target: Path):
    ...
    for config in protocol["tabnet_candidates"]:
        ...
        trials.append({"config": config, "epochs": int(model.best_epoch) + 1,
    ...
    for seed in protocol["seeds"]:
        ...
        predictions.append(model.predict_proba(transform.apply(test, embedding=True))[:, 1].astype(float))
...
def train_model(name, train, test, stage, protocol, target: Path):
    ...
    if name in ("logistic", "boosted_trees", "logistic_enriched"):
        prediction, fitted = fit_baseline(train, test, stage, "logistic" if full else name, full=full)
    ...
    info.update(model=name, stage=stage, trained_through=max(r["label_end"] for r in train),
                train_ids=[r["id"] for r in train], train_issuers=[r["issuer_id"] for r in train],
...
def predict_saved(target: Path, rows):
    ...
    if any(row["as_of"] <= info["trained_through"] or row["issuer_id"] in info["train_issuers"] for row in rows):
    ...
    if info["model"] in ("logistic", "boosted_trees", "logistic_enriched"):
        ...
        x = fitted["transform"].apply(rows) if fitted["transform"] else np.array(cat_matrix(rows, fitted["features"]), dtype=float)
