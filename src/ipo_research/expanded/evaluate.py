...
from .models import predict_saved, train_model
...
def stats(rows, name):
    ...
    return {
        ...
        "brier": brier,
        "brier_skill": 1 - brier / reference,
        "brier_skill_vs_constant_50": 1 - brier / 0.25,
        "training_rate_reference_brier": reference,
        "constant_50_reference_brier": 0.25,
        ...
        "calibration": bins,
        "screening": [
            screening(y, p, np.array([r["drawdown"] >= 0.3 for r in rows]), cutoff)
            for cutoff in (0.2, 0.3, 0.5)
        ],
    }
...
def paired_intervals(rows, name, replicates=2000):
    ...
    for _ in range(replicates):
        ...
        samples.append([loss, 1 - loss / reference, loss - linear, 1 - loss / 0.25])
    ...
    return {
        ...
        for i, key in enumerate(
            (
                "brier_ci",
                "brier_skill_ci",
                "brier_difference_vs_logistic_ci",
                "brier_skill_vs_constant_50_ci",
            )
        )
    }
...
def train(directory: Path, artifact_dir: Path, stages=(0, 20), *, sensitivity=False) -> dict:
    ...
    training_hashes = {
        str(p.relative_to(source.parent)): sha(p)
        for p in [
            source / "models.py",
            source / "dataset.py",
            source.parent / "models.py",
            source.parent / "dataset.py",
        ]
    }
    provenance = {
        "dataset_sha256": sha(directory / "dataset.json.gz"),
        "protocol_sha256": sha(protocol_path),
        "training_source_sha256": training_hashes,
        "packages": {
            name: version(name)
            for name in ("numpy", "scikit-learn", "catboost", "pytorch-tabnet", "torch")
        },
    }
    reports = {}
    for stage in stages:
        ...
        for fold in temporal_folds(rows, protocol):
            ...
            probability = {
                "base_rate": np.full(len(test), np.mean([r["event"] for r in train_rows]))
            }
            model_reports = {}
            for name in names:
                ...
                if cached and cached["cache_key"] == cache_key:
                    ...
                    save_json(
                        checkpoint,
                        {"cache_key": cache_key, "probabilities": p.tolist(), "info": info},
                    )
                probability[name] = p
                model_reports[name] = {
                    key: value
                ...
                if info.get("seed_predictions"):
                    model_reports[name]["seed_brier"] = [
                        float(brier_score_loss([r["event"] for r in test], values))
                        for values in info["seed_predictions"]
                    ]
                print(
                    f"Stage {stage}, fold {fold['id']}, {name}: fit/reload verified in {time.monotonic() - started:.1f}s",
                    flush=True,
                )
            records = [
                {
                    **{key: value for key, value in row.items() if key != "features"},
                    "fold": fold["id"],
                    "prior_mvp_case": (row["symbol"], row["listing_date"]) in old_cases,
                    "probabilities": {
                        name: float(values[i]) for name, values in probability.items()
                    },
                }
            ...
            folds.append(
                {
                    ...
                    "models": {name: stats(records, name) for name in probability},
                    "training": model_reports,
                }
            )
            save_json(
                directory / f"stage-{stage}-progress.json",
                {"folds": folds, "predictions": predictions},
            )
        metrics = {
            name: {
                **stats(predictions, name),
                **paired_intervals(predictions, name, protocol["bootstrap_replicates"]),
            }
    ...
    save_json(
        directory / ("coverage-results.json.gz" if sensitivity else "results.json.gz"), result
    )
    return result
