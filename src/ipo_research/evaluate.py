...
from .dataset import FEATURES, EVENT_THRESHOLD, HORIZON_SESSIONS, OBSERVATION_SESSIONS, build_dataset
from .models import MODEL_CONFIG, export_logistic, fit_models, matrix, predict_frozen

MODEL_NAMES = {"base_rate": "Training event rate", "logistic": "Logistic regression", "boosted_trees": "Shallow boosted trees"}
...
def temporal_folds(rows: list[dict], count: int = 3) -> list[dict]:
    ...
    for index, block in enumerate(np.array_split(evaluation_dates, count)):
        ...
        if len(train) < 20 or len({row["event"] for row in train}) < 2:
            raise ValueError(f"Fold {index + 1} needs 20 mature training observations and both classes")
        result.append({"id": index + 1, "start": start, "end": end, "train": train,
                       "test": test, "purged": len(candidates) - len(train)})
    return result
...
def screening(y: np.ndarray, p: np.ndarray, severe: np.ndarray, threshold: float) -> dict:
    ...
    return {"threshold": threshold, "flagged": flags, "flagged_share": float(flagged.mean()),
            "events": events, "caught": true_positive,
            ...
            "missed_severe_rate": float((~flagged & severe).sum() / severe_count) if severe_count else None}


def skill_interval(y: np.ndarray, p: np.ndarray, baseline: np.ndarray, months: list[str]) -> list[float] | None:
    ...
    return np.quantile(skills, [.025, .975]).tolist() if skills else None
...
def metrics(predictions: list[dict], model: str, intervals: bool = True) -> dict:
    ...
    severe = np.array([row["drawdown"] >= .3 for row in predictions])
    ...
    for left, right in zip([0, .2, .4, .6, .8], [.2, .4, .6, .8, 1]):
        ...
        if mask.any():
            bins.append({"lower": left, "upper": right, "n": int(mask.sum()),
                         "mean_probability": float(p[mask].mean()), "event_rate": float(y[mask].mean())})
    return {"n": len(y), "events": int(y.sum()), "event_rate": float(y.mean()),
            "brier": bs, "brier_skill": 1 - bs / reference if reference > 0 else None,
            "brier_skill_interval": skill_interval(y, p, baseline, [row["as_of"][:7] for row in predictions]) if intervals else None,
            ...
            "calibration": bins,
            "screening": [screening(y, p, severe, cutoff) for cutoff in [.1, .2, .3, .4, .5]]}
...
def run_evaluation(input_path: Path, output_path: Path) -> dict:
    ...
    for fold in temporal_folds(rows):
        ...
        for i, row in enumerate(test):
            ...
            fold_predictions.append({**row, "fold": fold["id"], "drivers": drivers,
                "probabilities": {"base_rate": base_rate, **{name: float(values[i]) for name, values in probabilities.items()}}})
        predictions.extend(fold_predictions)
        reports.append({"id": fold["id"], "test_start": fold["start"], "test_end": fold["end"],
            "train_count": len(train), "test_count": len(test), "purged_count": fold["purged"],
            "train_label_end": frozen["trained_through"], "train_events": sum(row["event"] for row in train),
            "models": {name: metrics(fold_predictions, name, intervals=False) for name in MODEL_NAMES}})
    source_files = sorted(Path(__file__).parent.glob("*.py"))
    report = {
        "schema_version": "2.0.0", "primary_model": "logistic", "synthetic": False,
        "target": {"observation_sessions": OBSERVATION_SESSIONS, "horizon_sessions": HORIZON_SESSIONS,
        ...
        "provenance": {"input_sha256": hashlib.sha256(raw).hexdigest(),
            ...
            "retrieved_at": max([bundle["benchmark"]["retrieved_at"]] + [row["retrieved_at"] for row in bundle["listings"]]),
            "benchmark_source": bundle["benchmark"]["source_page"]},
        "cohort": {"requested": len(bundle["listings"]) + len(bundle["exclusions"]),
                   "eligible": len(rows), "evaluated": len(predictions), "exclusions": exclusions,
                   "selection": bundle["selection"], "observation_start": rows[0]["as_of"], "observation_end": rows[-1]["as_of"]},
        "features": list(FEATURES), "model_config": MODEL_CONFIG,
        "protocol": {
        ...
        "models": [{"id": name, "name": title, **metrics(predictions, name)} for name, title in MODEL_NAMES.items()],
        "folds": reports, "predictions": predictions, "frozen_logistic_models": model_snapshots,
    }
