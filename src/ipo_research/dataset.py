...
def make_features(observed: list[dict], benchmark: list[dict]) -> dict[str, float]:
    ...
    benchmark = [
        row for row in benchmark if observed[0]["date"] <= row["date"] <= observed[-1]["date"]
    ]
    ...
    for listing in bundle["listings"]:
        try:
            ...
            expected = [
                ...
                if bars[0]["date"] <= row["date"] <= bars[-1]["date"]
            ]
            ...
            future = np.array(
                [row["adjusted_close"] for row in bars[OBSERVATION_SESSIONS - 1 :]], dtype=float
            )
            ...
            snapshots.append(
                {
                    ...
                    "features": features,
                    "drawdown": severity,
                    "event": int(severity >= EVENT_THRESHOLD),
                    "path": [
                        {
                            "date": row["date"],
                            "value": row["adjusted_close"] / bars[19]["adjusted_close"] * 100,
                        }
                        for row in bars
                    ],
                    "source_url": listing["source_page"],
