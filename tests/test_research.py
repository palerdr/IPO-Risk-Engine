...
from ipo_research.dataset import FEATURES, build_dataset, drawdown, make_features
...
def bars(count=45):
    ...
    return [
        {"date": day.isoformat(), "adjusted_close": 100 + index, "volume": 1000 + index}
...
def listing_bundle():
    ...
    return {
        "exclusions": [],
        "benchmark": {"bars": deepcopy(series)},
        "listings": [
            {
                "symbol": "TEST",
                "sector": "test",
                "listing_date": series[0]["date"],
                "source_page": "https://example.test/prices",
...
def sample_rows(count=90):
    ...
    return [
        {
            "symbol": f"CASE{i}",
            "as_of": (start + timedelta(days=i * 3)).isoformat(),
...
class DatasetTests(unittest.TestCase):
    def test_drawdown_includes_the_prediction_close_and_respects_peak_order(self):
        self.assertAlmostEqual(drawdown(np.array([100, 80, 90])), 0.2)
        self.assertAlmostEqual(drawdown(np.array([100, 120, 90])), 0.25)
    ...
    def test_snapshot_uses_twenty_observations_and_twenty_future_sessions(self):
        ...
        self.assertAlmostEqual(rows[0]["drawdown"], 0.5)
    ...
    def test_future_prices_and_future_benchmark_values_do_not_enter_features(self):
        ...
        self.assertEqual(
            make_features(observed, bundle["benchmark"]["bars"]),
...
class EvaluationTests(unittest.TestCase):
    def test_split_purges_unmatured_labels_and_never_trains_on_test_issuers(self):
        ...
        for fold in folds:
            ...
            self.assertTrue(
                set(row["symbol"] for row in fold["train"]).isdisjoint(
                    row["symbol"] for row in fold["test"]
                )
            )
    ...
    def test_equal_dates_stay_in_one_test_block_and_duplicate_issuers_fail(self):
        ...
        assignments = {
            row["symbol"]: fold["id"] for fold in temporal_folds(rows) for row in fold["test"]
        }
    ...
    def test_screening_uses_explicit_denominators_and_no_flags_is_undefined(self):
        y, p = np.array([1, 0, 1, 0]), np.array([0.7, 0.8, 0.2, 0.1])
        severe = np.array([True, False, True, False])
        result = screening(y, p, severe, 0.5)
        self.assertEqual(result["precision"], 0.5)
        self.assertEqual(result["recall"], 0.5)
        self.assertEqual(result["missed_severe_rate"], 0.5)
        self.assertIsNone(screening(y, p, severe, 0.9)["precision"])
    ...
    def test_brier_uses_each_folds_training_rate_baseline(self):
        predictions = [
            ...
            for event, base, p in [(1, 0.2, 0.8), (0, 0.4, 0.1)]
        ]
        result = metrics(predictions, "logistic", intervals=False)
        self.assertAlmostEqual(result["brier"], 0.025)
        self.assertAlmostEqual(result["brier_skill"], 1 - 0.025 / 0.4)
...
class FrozenStudyTests(unittest.TestCase):
    ...
    def test_report_matches_a_fresh_run_from_the_frozen_real_data(self):
        ...
        self.assertEqual(
            actual["provenance"]["input_sha256"],
